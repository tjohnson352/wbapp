...
def structure_data(df1):
    ...
    ### Create DataFrame df1a from Extracted Data in df1
    ...
    # Extract start_time and end_time as plain strings in HH:MM format
    df1a['start_time'] = df1a['timespan'].apply(lambda x: x.split(' - ')[0].strip() if ' - ' in x else '')
    df1a['end_time'] = df1a['timespan'].apply(lambda x: x.split(' - ')[1].strip() if ' - ' in x else '')

    # Ensure start_time and end_time remain as strings in HH:MM format
    df1a['start_time'] = df1a['start_time'].apply(lambda x: pd.to_datetime(x, format='%H:%M').strftime('%H:%M') if x else '')
    df1a['end_time'] = df1a['end_time'].apply(lambda x: pd.to_datetime(x, format='%H:%M').strftime('%H:%M') if x else '')
    ...
    df1a.loc[df1a['type'] == 'TEACHING', 'activities'] = (
        df1a.loc[df1a['type'] == 'TEACHING', 'year_group'].str.upper() + " " +
        df1a.loc[df1a['type'] == 'TEACHING', 'activities']
    )
